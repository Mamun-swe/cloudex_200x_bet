  ...
  $statemen = $pdo->prepare("SELECT * FROM tbl_member WHERE user_id=?");
    ...
    if(!empty($club_id)){
        ...
        //	$club_credit = $rows['credit'];
        	$co_id = $rows['club_owner_id'];
        }
    }
    if(!empty($co_id)){
        $statedc = $pdo->prepare("SELECT * FROM tbl_member WHERE user_id=?");
        $statedc->execute(array($co_id));
        $totalsc = $statedc->rowCount();
        $resulc = $statedc->fetchAll(PDO::FETCH_ASSOC);
        foreach ($resulc as $rowsc) {
        	$club_credit = $rowsc['credit'];
        	$cu_id = $rowsc['user_id'];
        }    
        ...
        if($totals > 0){
            ...
            $statement6 = $pdo->prepare("UPDATE tbl_member SET credit=? WHERE user_id=?");
            $statement6->execute(array($final_club_amounts,$cu_id));
            
            //enter commision table
            $date=date("Y-m-d h:i");
            $statement7 = $pdo->prepare("INSERT INTO tbl_commission (commission_to, amount, commission_date) VALUES (?,?,?)");
            $statement7->execute(array($cu_id,$club_amount,$date));
            $last_id = $pdo->lastInsertId();
